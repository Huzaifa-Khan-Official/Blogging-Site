...
export const updatePost = async (req, res) => {
    try {
        ...
        const post = await Post.findById(id).select("slug");
        const baseSlug = title.replace(/ /g, "-").toLowerCase();

        const isSameSlug = post.slug == baseSlug;
        ...
        const updatedPost = await Post.findOneAndUpdate(
            { _id: post._id, user: user._id },
            { title, slug, ...req.body },
            { new: true }
        ).populate("user", "username img title");
